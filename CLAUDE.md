...
- Phase 8 **DONE** (2026-07-09, after Fede upgraded to Paid plan): full dev cycle on real AWS executed and DESTROYED. Verified: O2 (env via pipeline), O3 (scaling 1→2→1, zero-downtime, both dispatchers healthy in ALB), O4 (all workflows via OIDC), O5 (replication+flush per pair, admin rotated+verified, Tier-2 backup in S3, DLM policy ENABLED), O6 (public URL through ALB→dispatcher→publish, cached). Evidence in `docs/report/evidence/fase8-despliegue-aws.md` incl. incident log (5 real bugs found+fixed: Free-plan block, 30GB AMI min root, systemd fork kill → java -nofork foreground, user-data/EBS-attach race → device wait, harden-before-replication ordering). Global plumbing (`terraform/global` via `bootstrap-aws.yml`): OIDC+roles+tfstate bucket+lock+seed bucket+$50 budget. Post-destroy: only seed+tfstate buckets remain (~cents/mo). Buckets now `force_destroy` (prod backup bucket excluded) after BucketNotEmpty on first destroy
- Phase 8 leftovers/notes: dev AEM admin password is in scratchpad `aem_admin_pass` + GitHub dev env secret AEM_ADMIN_PASSWORD (env destroyed, so informational). Re-lift for exam demo: dispatch deploy-infra(dev,apply) → wait ~20min → deploy-app → configure(harden as desired) — BUT update dev env vars BINARIES_BUCKET/BACKUP_BUCKET after apply (bucket suffixes are random) and REMOVE the AEM_ADMIN_PASSWORD secret before first deploy-app (instances boot admin/admin; harden sets it)
- Phase 9 CONTENT DONE (2026-07-09): all 12 report chapters drafted in Spanish in `docs/report/` (see INDICE.md — everything ✅, incl. 11-resultados with real measured times/costs and 12-conclusiones). Remaining for the deliverable: Fede reviews the Spanish text → assemble final .docx (use the docx skill; needs rendered diagrams from docs/diagrams/) → Phase 10 slides (.pptx skill, from report content)
- NEXT: Fede's review of docs/report/*.md → .docx assembly → Phase 10 presentation
...
- Terraform CANNOT run against AWS from this Mac: aws-sdk-go requests to AWS APIs get NO responses (endless retries) while curl/python/aws-CLI and even Go→GitHub work. Undiagnosed (not DNS/IPv6/proxy/sandbox/HTTP2). Workaround: ALL terraform-against-AWS runs on GitHub runners (`bootstrap-aws.yml` for the global root; deploy-infra for envs). aws CLI locally is fine for verification/uploads
- AWS "Free plan" accounts (new account model) can ONLY launch free-tier-eligible instance types regardless of vCPU quota — RunInstances fails with `InvalidParameterCombination: not eligible for Free Tier`. AEM sizes need the Paid plan upgrade (root/billing access, Fede-only)
- Terraform `count` must be known at plan time — never gate on a computed attribute (e.g. a bucket ARN from another module); pass a separate boolean flag (see author module backup_bucket_write_enabled)
