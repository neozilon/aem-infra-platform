...
- Phase 8 SETUP DONE, DEPLOY BLOCKED on AWS account plan (2026-07-09). Done: `terraform/global` applied via one-shot `bootstrap-aws.yml` (OIDC provider, roles gha-aem-{dev,stage,prod} scoped to repo environments, tfstate bucket + lock table, seed bucket, $50 budget) — account 599526349046, us-east-1; binaries uploaded to seed bucket; S3 backends enabled; per-env AWS_ROLE_ARN + BINARIES_SEED_BUCKET vars set; vCPU quota raised 5→16; OIDC chain PROVEN (plan via runner works; also caught+fixed a count-on-computed-ARN bug). **BLOCKER: account is on AWS "Free plan" → only 1GB micro instances allowed; AEM needs t3.xlarge. Fede will upgrade to Paid plan later (console → Billing → Upgrade).** Partial dev env DESTROYED (verified: no EC2/NAT/ALB/EIP/VPC left; only tfstate+seed buckets remain). RESUME once upgraded: dispatch deploy-infra (dev/apply) → deploy-app → configure → backup → scaling demo 1→2→1 → evidence → destroy. Set secrets.AEM_ADMIN_PASSWORD + env vars BINARIES_BUCKET/BACKUP_BUCKET (from terraform outputs) before deploy-app/configure/backup
- NEXT: Phase 9 (Spanish report — sections for phases 2-7 drafted in docs/report/), Phase 10 (slides). Phase 8 deploy resumes when Fede upgrades the AWS plan
